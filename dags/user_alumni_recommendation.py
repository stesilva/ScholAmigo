...
#query similar user based on the retrived embedding
def query_pinecone_similarity(index, query_embedding: List[float], query_email, query_scholarship, top_k: int = 6):
    try:
        response = index.query(
            ...
            filter={
                "$and": [
                    {"status": {"$eq": "alumni"}},
                    {"scholarship_id": {"$eq": query_scholarship}}
                ]
            })
        logger.info(f"Top similar alumni recipient of the scholarship '{query_scholarship}' for '{query_email}':")
        for match in response["matches"]:
            if match['id'] != query_email:
...
def main():
    ...
    #similarity search example (simulated a clicked scholarship and the user that clicked)
    query_email = "quinnsherri@example.com"
    query_scholarship = "Global Innovators Grant"
    query_embedding = get_user_embedding(session, query_email, EMBEDDING_PROPERTY)
    if query_embedding:
        query_pinecone_similarity(pinecone_index, query_embedding, query_email, query_scholarship, top_k=6)   
